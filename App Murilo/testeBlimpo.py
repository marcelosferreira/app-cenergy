...
class FormularioPassoAPasso:
    ...
    def criar_etapa3(self):
        ...
        conteudo_frame.grid(row=1, column=0, sticky="nsew")
        ...
        def abrir_janela_edicao(linha):
            ...
            entry_vars = []
            titulos = ["CAPEX [USD]", "OPEX [USD/Ano]", "Pagamento de TUSTg [valor em USD/kWh]", "Geração no mesmo local do consumo [sim/não]", "Previsão da geração [kWh/h]", "Preço da Energia [USD/kWh]"]
            for col in range(6):
        ...
        tabela = [["" for _ in range(6)] for _ in range(6)]

        label_var = []

        # Adicione a primeira linha com títulos
        titulos = ["CAPEX [USD]", "OPEX [USD/Ano]", "Pagamento de TUSTg [valor em USD/kWh]", "Geração no mesmo local do consumo [sim/não]", "Previsão da geração [kWh/h]", "Preço da Energia [USD/kWh]"]
        label_var.append([tk.StringVar() for _ in range(6)])
        ...
        for linha in range(1, 6):
            ...
            for coluna in range(6):
                ...
                ttk.Label(conteudo, textvariable=label_var[linha][coluna]).grid(row=linha, column=coluna, padx=10, pady=5)

            botao_editar = ttk.Button(conteudo, text="Editar", command=lambda linha=linha: abrir_janela_edicao(linha))
            botao_editar.grid(row=linha, column=6, padx=10, pady=5)
